"""
Ultimate Orbital Globe: Earth Textured, Labeled, Animated, Exportable
"""
...
def create_earth_surface(resolution=100):
...
def main():
    ...
    parser.add_argument("--html", default="orbit_dashboard.html", help="Output HTML file")
    ...
    for sat_id, sat in satellites:
        ...
        fig.add_trace(go.Scatter3d(
            ...
            name=f"SAT {sat_id}",
            marker=dict(size=4),
            hovertext=[f"{sat_id}<br>{lbl}" for lbl in labels],
            hoverinfo='text'
        ))

    # Earth sphere (dark-glow style)
    ...
    fig.update_layout(
        title="🛰️ 3D Orbital Globe - Dashboard View",
        scene=dict(
            xaxis_title='X (km)',
            yaxis_title='Y (km)',
            zaxis_title='Z (km)',
    ...
    print(f"✅ Orbital dashboard saved to: {args.html}")
