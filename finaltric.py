...
if master_id in tle_dict and target_id in tle_dict:
    ...
    # --- Tab 1 ---
    with tabs[1]:
        st.markdown(f"### 4-Panel View for: {master_name} (Master) vs {target_name} (Target)")
        ...
        axs[0, 0].set_title("Cross vs In-Track")
        ...
        axs[0, 1].annotate(f'Min: {dist[min_idx]:.2f} km', xy=(times[min_idx], dist[min_idx]),
        ...
        axs[1, 0].set_title("Radial vs Cross")
    ...
    # --- Tab 2 ---
    with tabs[2]:
        st.markdown(f"### 3D Orbit for: {master_name} (Master) vs {target_name} (Target)")
    ...
    # --- Tab 4 ---
        st.markdown(f"### Deviation Alerts for: {master_name} vs {target_name}")
    with tabs[4]:
        dist = np.linalg.norm(ric, axis=1)
    ...
    # --- Tab 5 ---
        st.markdown(f"### 3D RIC Deviation View for: {master_name} vs {target_name}")
    with tabs[5]:
        from plotly.subplots import make_subplots

        fig_ric_3d = make_subplots(rows=2, cols=2,
                                   specs=[[{'type': 'scene'}, {'type': 'scene'}],
                                          [{'type': 'scene'}, {'type': 'scene'}]],
                                   subplot_titles=("Radial vs In-Track", "Cross vs In-Track",
                                                   "Radial vs Cross", "Full 3D RIC Vector"))

        r_vals = ric[start_idx:end_idx+1, 0]
        i_vals = ric[start_idx:end_idx+1, 1]
        c_vals = ric[start_idx:end_idx+1, 2]

        fig_ric_3d.add_trace(go.Scatter3d(x=r_vals, y=i_vals, z=[0]*len(r_vals),
                                          mode='lines+markers', name='R-I'), row=1, col=1)

        fig_ric_3d.add_trace(go.Scatter3d(x=c_vals, y=i_vals, z=[0]*len(c_vals),
                                          mode='lines+markers', name='C-I'), row=1, col=2)

        fig_ric_3d.add_trace(go.Scatter3d(x=r_vals, y=c_vals, z=[0]*len(r_vals),
                                          mode='lines+markers', name='R-C'), row=2, col=1)

        fig_ric_3d.add_trace(go.Scatter3d(x=r_vals, y=i_vals, z=c_vals,
                                          mode='lines+markers', name='Full RIC'), row=2, col=2)

        fig_ric_3d.update_layout(
            height=900,
            title_text="RIC Deviation Subplots (3D View)",
            scene=dict(camera=dict(eye=dict(x=1.2, y=1.2, z=0.8))),
            margin=dict(l=0, r=0, b=0, t=40),
            showlegend=True
        )
